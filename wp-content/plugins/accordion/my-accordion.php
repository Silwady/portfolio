<?php
/**
  Plugin Name: WPBeginner's  Accordion
...
// Generating Output
    ...
    foreach ($posts as $post) { // Generate the markup for each Question
        $faq .= sprintf(('<h3><a href="">%1$s</a></h3><div>%2$s</div>'), $post->post_title, wpautop($post->post_content)
        );
    }
    $faq .= '</div>'; //Close the container
    return $faq; //Return the HTML.
}
add_shortcode('android_accordion', 'accordion_shortcode');
